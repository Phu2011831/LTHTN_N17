...
static TaskHandle_t mdns_task_handle = NULL;
...
{	
    ...
    *dst++ = '\0';
    ...
    if((ret = mdns_init()) != ESP_OK){
        ESP_LOGE(TAG, "mdns_init() Failed, error code = 0x%x", ret);
        return;
    }
    ...
    xTaskCreate(mdns_task, "mdns-cam", 2048, NULL, 2, &mdns_task_handle);
}
...
void app_mdns_stop()
{
    ...
    if (mdns_task_handle != NULL) {
        vTaskDelete(mdns_task_handle);
        mdns_task_handle = NULL;  // Clear the handle after deletion
    }    
    ...
    if (query_lock != NULL) {
        vSemaphoreDelete(query_lock);
        query_lock = NULL;
    }

    ESP_LOGI(TAG, "mDNS stopped and resources freed.");
}
